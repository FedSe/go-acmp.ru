...
Кстати, по памяти `scanner.Scan()` как правило уделывает `Fscan(reader, ...)` на больших данных и проигрывает на малых, но тут дело больше в самих интерфейсах и их работе с буфером. На практике это не важно.
