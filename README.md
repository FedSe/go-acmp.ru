...
```go
t := 1000000000 // int, 13 симв.
t := 1 << 30    // int, 8 симв. = 1 * 2^30 = 1 073 741 824
```
...
Кстати, вывод нескольких **Int** через `Println(&u, &v)` избыточен, `Print(&u, &v)` выводит аналогично через пробел.
...
```go
// самостоятельная реализация - 32 символа в лучшем сценарии
for _, c := range s {
    n = n*10 + int(c-48)   // допустим, пустая n у нас уже есть
}
// либо ещё более медленная 
Sscan(s.Text(), &f)        // 18 символов

// реализация с импорта - 24 символа в худшем сценарии
( . "strconv" )            // допустим, кроме fmt не было ничего, тогда +2 скобки
n, _ := Atoi(s) 
```
`Sscan()` медленный настолько, что его применение возможно слишком редко. Если конвертация вызывается в коде 2 раза - `Atoi()` и `Sscan()` стоят одинаково
